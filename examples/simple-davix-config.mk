...
$(eval $(call Configure_TargetFS,localhost/build-tools,$(BUILD_TOP),PATH,STRIP LDD))

$(eval $(call TargetFS_Install_Make,localhost/build-tools,syslinux-3.83,,default))
$(eval $(call TargetFS_Install_Autoconf,localhost/build-tools,pkg-config-0.23 Python-2.6.1 autoconf-2.64 libtool-2.2.4 automake-1.11,NOSTAGE NODESTDIR))

DAVIX_BUILD_PATH          := $(call Toolchain_Path,host-tools):$(PATH)
DAVIX_TOOLCHAIN_VERSIONS  := binutils-2.18 gcc-4.2-20070307 glibc-2.5 linux-2.6.28.7 gdb-6.8
...
$(eval $(call TargetFS_Template,davix/initramfs,$(shell pwd)/fs-template/DAVIX))
$(eval $(call TargetFS_Install_Make,davix/initramfs,$(DAVIX_BUSYBOX),RELEASE,BASIC SH))

DAVIX_INITRAMFS_TARGETFS_NAME := davix/initramfs-devel
